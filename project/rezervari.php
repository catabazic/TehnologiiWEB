<?php include 'php/referer.php'; ?>

<!DOCTYPE html>
<html lang="en">

<head>
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <meta charset="UTF-8">
    <meta http-equiv="X-UA-Compatible" content="IE=edge">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Green Tea House</title>
    <link rel="stylesheet" href="css/rezervari.css">
    <link href="https://cdn.jsdelivr.net/npm/remixicon@3.0.0/fonts/remixicon.css" rel="stylesheet">
</head>

<body>
    <div class = "container">
        <nav>
            <div class="site-logo">
              <img src="imagini/logo-teahouse.png" alt="logo" width="60" height="60">
            </div>
            <div class="navigare">
              <ul>
                <li><a href="index.php">Acasă</a></li>
                <li><a href="meniu.php">Meniu</a></li>
                <li><a href="evenimente.php">Evenimente</a></li>
                <li><a href="despre.php">Despre</a></li>
                <li><a href="ajutor.php">Ajutor</a></li>
                <li><a href="rezervari.php">Rezervări</a></li>
              </ul>
              <div class="cart-icon">
                <a href="cos.php"><i class="ri-shopping-cart-2-line" style="font-size: 1.5em;"></i></a>
              </div>
            </div>
          </nav>
          
        
        <main> 
            <div class = "container-main">
                <div class = "anunt">
                    <h3>Te rugăm ca atunci când ocupi un loc în cafenea, să îl marchezi ca și ocupat. Mulțumim!</h3>
                    <p>Poți face asta prin apăsarea căsuței goale din dreptul locului pe care l-ai ocupat.</p>
                </div>
                <div class = "mese">
                    <div class="mese-de-4">
                        <div class="Masa1">
                            <h1>Masa 1 - 4 locuri</h1>
                            <div class="locuri">
                              <label><input type="checkbox" name="locul1">Locul 1</label>
                              <br>
                              <label><input type="checkbox" name="locul2">Locul 2</label>
                              <br>
                              <label><input type="checkbox" name="locul3">Locul 3</label>
                              <br>
                              <label><input type="checkbox" name="locul4">Locul 4</label>
                            </div>
                          </div>
                        <div class="Masa2">
                            <h1>Masa 2 - 4 locuri</h1>
                            <div class="locuri">
                            <label><input type="checkbox" name="locul1">Locul 1</label>
                            <br>
                            <label><input type="checkbox" name="locul2">Locul 2</label>
                            <br>
                            <label><input type="checkbox" name="locul3">Locul 3</label>
                            <br>
                            <label><input type="checkbox" name="locul4">Locul 4</label>
                            </div>
                        </div>
                        <div class="Masa3">
                            <h1>Masa 3 - 4 locuri</h1>
                            <div class="locuri">
                            <label><input type="checkbox" name="locul1">Locul 1</label>
                            <br>
                            <label><input type="checkbox" name="locul2">Locul 2</label>
                            <br>
                            <label><input type="checkbox" name="locul3">Locul 3</label>
                            <br>
                            <label><input type="checkbox" name="locul4">Locul 4</label>
                            </div>
                        </div>
                        <div class="Masa4">
                            <h1>Masa 4 - 4 locuri</h1>
                            <div class="locuri">
                            <label><input type="checkbox" name="locul1">Locul 1</label>
                            <br>
                            <label><input type="checkbox" name="locul2">Locul 2</label>
                            <br>
                            <label><input type="checkbox" name="locul3">Locul 3</label>
                            <br>
                            <label><input type="checkbox" name="locul4">Locul 4</label>
                            </div>
                        </div>
                    </div>
                    <div class="mese-de-5">
                        <div class="Masa5">
                            <h1>Masa 5 - 4 locuri</h1>
                            <div class="locuri">
                            <label><input type="checkbox" name="locul1">Locul 1</label>
                            <br>
                            <label><input type="checkbox" name="locul2">Locul 2</label>
                            <br>
                            <label><input type="checkbox" name="locul3">Locul 3</label>
                            <br>
                            <label><input type="checkbox" name="locul4">Locul 4</label>
                            </div>
                        </div>
                        <div class="Masa6">
                            <h1>Masa 6 - 4 locuri</h1>
                            <div class="locuri">
                            <label><input type="checkbox" name="locul1">Locul 1</label>
                            <br>
                            <label><input type="checkbox" name="locul2">Locul 2</label>
                            <br>
                            <label><input type="checkbox" name="locul3">Locul 3</label>
                            <br>
                            <label><input type="checkbox" name="locul4">Locul 4</label>
                            </div>
                        </div>
                        <div class="Masa7">
                            <h1>Masa 7 - 5 locuri</h1>
                            <div class="locuri">
                            <label><input type="checkbox" name="locul1">Locul 1</label>
                            <br>
                            <label><input type="checkbox" name="locul2">Locul 2</label>
                            <br>
                            <label><input type="checkbox" name="locul3">Locul 3</label>
                            <br>
                            <label><input type="checkbox" name="locul4">Locul 4</label>
                            <br>
                            <label><input type="checkbox" name="locul5">Locul 5</label>
                            </div>
                        </div>
                        <div class="Masa9">
                            <h1>Masa 8 - 6 locuri</h1>
                            <div class="locuri">
                            <label><input type="checkbox" name="locul1">Locul 1</label>
                            <br>
                            <label><input type="checkbox" name="locul2">Locul 2</label>
                            <br>
                            <label><input type="checkbox" name="locul3">Locul 3</label>
                            <br>
                            <label><input type="checkbox" name="locul4">Locul 4</label>
                            <br>
                            <label><input type="checkbox" name="locul5">Locul 5</label>
                            <br>
                            <label><input type="checkbox" name="locul6">Locul 6</label>
                            </div>
                        </div>
                    </div>
                </div>
                <div class="numar-locuri">
                    <h4>Numărul de locuri: 35</h4>
                    <h4>Numărul de locuri ocupate: </h4>
                    <h4>Numărul de locuri libere: </h4>
                </div>
            </div>
        </main>
    </div>
    <div class = "container-footer">
        <footer>
            <h3>Program:</h3>
            <p>Luni: 09:00-22:00</p>
            <p>Marți: 09:00-22:00</p>
            <p>Miercuri: 09:00-22:00</p>
            <p>Joi: 09:00-22:00</p>
            <p>Vineri: 09:00-22:00</p>
            <p>Sâmbătă: 09:00-22:00</p>
            <p>Duminică: 09:00-22:00</p>
        </footer>
    </div>
</body>


</html>